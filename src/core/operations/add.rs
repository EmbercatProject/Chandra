...
use crate::core::{
    operation::{Operation, OperationWrapper, Differentiable}, processor::cpu::DifferentiatedCPUContext, type_traits::Calculatable,
};

pub fn add<R: Calculatable, LEFT: Operation<R>, RIGHT: Operation<R>>(
    left: LEFT,
    right: RIGHT,
) -> OperationWrapper<R, Add<R, LEFT, RIGHT>> {
    OperationWrapper(
        Add {
            left: left,
            right: right,
...
#[derive(Clone, Debug)]
pub struct Add<R: Calculatable, LEFT: Operation<R>, RIGHT: Operation<R>> {
    pub left: LEFT,
    pub right: RIGHT,
    _0: PhantomData<R>,
}
...
impl<R: Calculatable, LEFT: Differentiable<R>, RIGHT: Differentiable<R>> Differentiable<R> for Add<R, LEFT, RIGHT> {
    type Diff = OperationWrapper<R, Add<R, LEFT::Diff, RIGHT::Diff>>;

    fn auto_diff_for<R1: Clone>(&self, var: super::var::Variable<R1>, var_trace: &mut HashMap<String, Vec<String>>) -> Self::Diff {
        add(self.left.auto_diff_for(var.clone(), var_trace), self.right.auto_diff_for(var.clone(), var_trace))
    }
